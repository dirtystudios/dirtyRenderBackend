...
BufferId MetalDevice::AllocateBuffer(const BufferDesc& desc, const void* initialData) {
    ...
    dg_assert(desc.isDynamic == false, "Dynamic buffers are unsupported");
...
PipelineStateId MetalDevice::CreatePipelineState(const PipelineStateDesc& desc) {
    ...
    if (renderPass->info.hasDepth()) {
        rpd.depthAttachmentPixelFormat = MetalEnumAdapter::toMTL(renderPass->info.depthAttachment().value().format);
    }
    
    if (renderPass->info.hasStencil()) {
        rpd.stencilAttachmentPixelFormat = MetalEnumAdapter::toMTL(renderPass->info.stencilAttachment().value().format);
    }
    
    for (const ColorAttachmentDesc& attachment : renderPass->info.colorAttachments()) {
        size_t i = attachment.index;
        rpd.colorAttachments[i].pixelFormat         = MetalEnumAdapter::toMTL(attachment.format);
...
TextureId MetalDevice::CreateTexture2D(PixelFormat format, const TextureUsageFlags& usage, uint32_t width, uint32_t height, void* srcData, const std::string& debugName) {
...
uint8_t* MetalDevice::MapMemory(BufferId bufferId, const BufferAccess& access) {
    MetalBuffer* buffer = _resourceManager->GetResource<MetalBuffer>(bufferId);
    if (buffer->desc.accessFlags.test(BufferAccessBitPositionCpuWrite) == false) {
        return nullptr;
    }
